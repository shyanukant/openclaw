...
# [Gateway Initialization] Force Gemini, bind to 0.0.0.0, and map to Render's $PORT
CMD ["sh", "-c", "node openclaw.mjs config set agents.defaults.model.primary \"google/gemini-2.5-flash\" && node openclaw.mjs config set gateway.auth.token \"${OPENCLAW_GATEWAY_TOKEN}\" && node openclaw.mjs config set gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback true && node openclaw.mjs config set gateway.controlUi.dangerouslyDisableDeviceAuth true && node openclaw.mjs config set gateway.trustedProxies '[\"0.0.0.0/0\"]' && echo \"Gateway Token: ${OPENCLAW_GATEWAY_TOKEN}\" && exec node openclaw.mjs gateway --bind 0.0.0.0 --port ${PORT} --allow-unconfigured"]
